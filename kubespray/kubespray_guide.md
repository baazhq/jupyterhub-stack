...
The inventory defines your cluster topology. Kubespray provides a sample inventory structure.

#### Option A: Copy Sample Inventory (Recommended)

```bash
# Create your cluster inventory from sample
...
│       ├── addons.yml        # Optional add-ons
│       └── k8s-net-*.yml     # Network plugin configs
└── inventory.ini             # Node definitions
```

#### Option B: Use Inventory Builder (For Many Nodes)

```bash
# Declare IPs of your nodes (use PRIVATE/internal IPs)
declare -a IPS=(10.0.0.1 10.0.0.2 10.0.0.3 10.0.0.4 10.0.0.5)

# Generate inventory file using the inventory builder script
# NOTE: Use the contrib script, NOT ansible-inventory
CONFIG_FILE=inventory/mycluster/hosts.yml python3 contrib/inventory_builder/inventory.py ${IPS[@]}
```

> [!WARNING]
> **Common Mistake**: Do NOT use `ansible-inventory` or `ansible/cli/inventory.py`. 
> The correct script is `contrib/inventory_builder/inventory.py`.

---

### 2.2 Configure Inventory File
...
master2 ansible_host=192.168.1.11 ip=192.168.1.11 etcd_member_name=etcd2
master3 ansible_host=192.168.1.12 ip=192.168.1.12 etcd_member_name=etcd3
...
master2 ansible_host=203.0.113.11 ip=10.0.0.2 etcd_member_name=etcd2
master3 ansible_host=203.0.113.12 ip=10.0.0.3 etcd_member_name=etcd3
...
master2 ansible_host=192.168.1.11 etcd_member_name=etcd2
master3 ansible_host=192.168.1.12 etcd_member_name=etcd3
...
### 2.4 Network Plugin Selection

#### Calico (Default)

Edit `group_vars/k8s_cluster/k8s-net-calico.yml`:

```yaml
calico_ipip_mode: 'CrossSubnet'  # or 'Always' or 'Never'
calico_vxlan_mode: 'Never'
calico_network_backend: 'bird'    # or 'vxlan'
```

#### Cilium

Edit `group_vars/k8s_cluster/k8s-net-cilium.yml`:
...
### 2.5 Configure Add-ons (Optional)
...
ls -la inventory/mycluster/artifacts/
...
master2    Ready    control-plane   10m   v1.30.0
master3    Ready    control-plane   10m   v1.30.0
...
### 4.2 Adding Control Plane Nodes

> [!CAUTION]
> Adding control plane nodes is more complex. Use `cluster.yml`, not `scale.yml`.
...
master2 ansible_host=192.168.1.11 etcd_member_name=etcd2
master3 ansible_host=192.168.1.12 etcd_member_name=etcd3
master4 ansible_host=192.168.1.13 etcd_member_name=etcd4  # NEW
```

#### Step 2: Run Cluster Playbook

```bash
# Must use cluster.yml for control plane changes
ansible-playbook -i inventory/mycluster/inventory.ini cluster.yml \
  -b \
  -v \
  --private-key=~/.ssh/id_rsa
...
#### Step 3: Restart nginx-proxy on All Nodes

The local nginx proxy on each node needs to be restarted to learn about the new control plane:

```bash
# For containerd runtime
ansible all -i inventory/mycluster/inventory.ini -m shell \
  -a "crictl ps | grep nginx-proxy | awk '{print \$1}' | xargs crictl stop" \
  -b

# For docker runtime
ansible all -i inventory/mycluster/inventory.ini -m shell \
  -a "docker ps | grep k8s_nginx-proxy | awk '{print \$1}' | xargs docker restart" \
  -b
```

**Purpose**: Each node has a local nginx proxy that load balances to control planes. Restarting refreshes its configuration.

---

### 4.3 Adding etcd Nodes

> [!WARNING]
> Always maintain an **odd number** of etcd nodes (3, 5, 7) for quorum.

```bash
# Add etcd node and run with special flags
ansible-playbook -i inventory/mycluster/inventory.ini cluster.yml \
  -b \
  --limit=etcd,kube_control_plane \
  -e ignore_assert_errors=yes

# Then run upgrade to sync configuration
...
### 5.2 Removing Control Plane Nodes

> [!CAUTION]
> **CRITICAL**: You cannot remove the **first control plane node** directly. See Special Cases below.

```bash
# Remove a non-first control plane node
ansible-playbook -i inventory/mycluster/inventory.ini remove-node.yml \
  -b \
  -e "node=master2"

# Then update inventory and run cluster.yml
ansible-playbook -i inventory/mycluster/inventory.ini cluster.yml -b
```

#### Special Case: Removing the First Control Plane Node

1. **Reorder inventory** to make another node first:

   ```ini
   # Before (master1 is first - want to remove)
   [kube_control_plane]
   master1 ansible_host=192.168.1.10
   master2 ansible_host=192.168.1.11
   master3 ansible_host=192.168.1.12
   
   # After (master2 is now first)
   [kube_control_plane]
   master2 ansible_host=192.168.1.11
   master3 ansible_host=192.168.1.12
   master1 ansible_host=192.168.1.10
...
2. **Run upgrade to apply new ordering**:
   ```bash
...
4. **Update cluster-info ConfigMap**:
   ```bash
   kubectl edit cm -n kube-public cluster-info
   # Update the 'server' field to point to a remaining control plane IP
   ```

---

### 5.3 Removing etcd Nodes
...
# Remove from inventory, then regenerate cluster config
ansible-playbook -i inventory/mycluster/inventory.ini cluster.yml -b
```

After removal, update `/etc/kubernetes/manifests/kube-apiserver.yaml` on each control plane to remove the old etcd endpoint from `--etcd-servers=...`.
